...
def tbl():
    return Table(
        (("ozzy", "dog", 18), ("mary", "cat", 12)), columns=("name", "animal", "age")
    )
...
class TestTable:
    ...
    def test__eq__(self, tbl):
        ...
        assert tbl != Table(
            (("ozzy", "dog", 18), ("mary", "cat", 2)), columns=("name", "animal", "age")
        )

    def test__eq__sequence(self, tbl):
        assert tbl == (("ozzy", "dog", 18), ("mary", "cat", 12))

    def test__eq__int(self, tbl):
        assert tbl != 1

    def test__lt__(self, tbl):
        assert tbl < Table((("ozzy", "dog", 19),))
        assert not tbl < Table((("ozzy", "dog", 17),))
        with pytest.raises(TypeError):
            tbl < [("ozzy", "dog", 19)]

    def test__hash__(self, tbl):
        assert hash(tbl) == hash(
            (("name", "animal", "age"), ("ozzy", "dog", 18), ("mary", "cat", 12))
        )
    ...
    def test__repr__(self, tbl):
        assert (
            repr(tbl)
            == "Table([('ozzy', 'dog', 18), ('mary', 'cat', 12)], columns=('name', 'animal', 'age'))"
        )
...
class TestTable__getitem__:
    ...
    def test_row_slice(self, tbl):
        assert tbl[:1] == Table(
            (("ozzy", "dog", 18),), columns=("name", "animal", "age")
        )

    def test_negative_row_slice(self, tbl):
        assert tbl[-1:] == Table(
            (("mary", "cat", 12),), columns=("name", "animal", "age")
        )
...
class TestMutableTable:
    ...
    def test__setitem__slice(self, mtbl):
        ...
        assert tuple(mtbl[0]) == ("ozzy", "rabbit", 3)
        assert tuple(mtbl[1]) == ("mary", "spider", 1)

    def test__setitem__append_column(self, mtbl):
        mtbl[:, "color"] = ("white", "black")
        assert tuple(mtbl[0]) == ("ozzy", "dog", 18, "white")
        assert tuple(mtbl[1]) == ("mary", "cat", 12, "black")
        assert tuple(mtbl.columns) == ("name", "animal", "age", "color")

    def test__setitem__insert_column(self, mtbl):
        mtbl[:, "age":"age"] = Table([("black",), ("white",)], columns=("color",))
        assert tuple(mtbl[0]) == ("ozzy", "dog", "black", 18)
        assert tuple(mtbl[1]) == ("mary", "cat", "white", 12)
        assert tuple(mtbl.columns) == ("name", "animal", "color", "age")

    def test__delitem__rows(self, mtbl):
        del mtbl[0]
        assert tuple(mtbl[0]) == ("mary", "cat", 12)

    def test__delitem__columns(self, mtbl):
        del mtbl[:, "animal":]
        assert tuple(mtbl[0]) == ("ozzy",)
    ...
    def test_insert(self, mtbl):
        ...
        assert tuple(mtbl[0]) == ("john", "cat", 12)
        assert tuple(mtbl[1]) == ("ozzy", "dog", 18)
